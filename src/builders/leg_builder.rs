...
pub fn build_leg() -> MechanismState {
    ...
    let d_pelvis = l1;
    let h_pelvis = l2;
    let pelvis_axis: UnitVector3<Float> = Vector3::z_axis();
    ...
    let pelvis_left_to_world = Transform3D::identity(pelvis_left_frame, WORLD_FRAME);
    ...
    let treejoints = vec![
        Joint::new_floating(pelvis_left_to_world),
    ...
    let bodies = vec![pelvis_left, hip_left, thigh_left, calf_left, foot_left];
...
pub fn build_leg_from_foot() -> MechanismState {
    ...
    let d_pelvis = l1;
    let h_pelvis = l2;
    let pelvis_axis = -Vector3::y_axis();
    let pelvis_left_frame = "pelvis_left";
    let pelvis_com = vector![0., 0., h_pelvis / 2.];
    let pelvis_left = RigidBody::new_cuboid_at(
        &pelvis_com,
